...
export function FamilyUnitEditor({ open, unit, people, onClose, onSave, onDelete }: UnitProps) {
  ...
  useEffect(() => { if (open) { setDraft(unit ? { husbandId: unit.husbandId, wifeId: unit.wifeId, anniversaryDate: unit.anniversaryDate, childrenIds: [...unit.childrenIds] } : { husbandId: null, wifeId: null, anniversaryDate: null, childrenIds: [] }); setError(null); } }, [open, unit]);
  if (!open) return null;
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setBusy(true); setError(null); try { await onSave(draft); onClose(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to save.'); } finally { setBusy(false); } };
  ...
  const options = <><option value="">Not selected</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</>;
  return <div className="modal-backdrop" role="presentation" onMouseDown={onClose}><form className="editor-modal" onSubmit={(event) => void submit(event)} onMouseDown={(event) => event.stopPropagation()}>
    ...
    <div className="form-grid"><label className="field"><span>Partner / husband</span><select value={draft.husbandId ?? ''} onChange={(event) => setDraft({ ...draft, husbandId: event.target.value || null })}>{options}</select></label><label className="field"><span>Partner / wife</span><select value={draft.wifeId ?? ''} onChange={(event) => setDraft({ ...draft, wifeId: event.target.value || null })}>{options}</select></label><label className="field full"><span>Anniversary</span><input type="date" value={draft.anniversaryDate ?? ''} onChange={(event) => setDraft({ ...draft, anniversaryDate: event.target.value || null })} /></label><label className="field full"><span>Children</span><select multiple size={Math.min(7, Math.max(4, people.length))} value={draft.childrenIds} onChange={(event) => setDraft({ ...draft, childrenIds: [...event.target.selectedOptions].map((option) => option.value) })}>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select><small>Hold Command (Mac) or Ctrl (Windows) to select more than one.</small></label></div>
    {error && <div className="error-banner" role="alert">{error}</div>}
