...
export function Workspace({ identity }: { identity: UserIdentity }) {
  ...
  const availableViews = (Object.keys(VIEW_META) as View[]).filter((key) => isAdmin || ['tree', 'people', 'relationships'].includes(key));
...
function PeopleView({ tree, isAdmin, onSelect, onEdit, onDelete }: { tree: FamilyTreeData; isAdmin: boolean; onSelect: (id: string) => void; onEdit: (person: Person) => void; onDelete: (person: Person) => void }) {
  ...
  return <div className="content-page"><div className="section-intro"><div><p className="eyebrow">{tree.people.length} profiles</p><h2>People in your family</h2><p>{isAdmin ? 'Keep names and life details accurate. Connections are managed separately as family units.' : 'Browse the family profiles. Only administrators can make changes.'}</p></div><label className="inline-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter people…" /></label></div><div className="people-grid">{people.map((person) => <article className="person-card" key={person.id}><button className="person-card-main" onClick={() => onSelect(person.id)}><span className={`avatar large avatar-${person.gender}`}>{person.name[0]}</span><span><strong>{person.name}</strong><small>{person.currentPlaceOfResidence || 'Residence not recorded'}</small></span><ChevronRight size={16} /></button><div><span>{person.isAlive ? 'Living' : 'In memory'}</span>{isAdmin && <span className="row-actions"><button onClick={() => onEdit(person)}>Edit</button><button className="delete-link" onClick={() => onDelete(person)}><Trash2 size={12} /> Delete</button></span>}</div></article>)}</div></div>;
}
...
function PersonDrawer({ person, tree, isAdmin, onClose, onEdit, onFocus }: { person: Person; tree: FamilyTreeData; isAdmin: boolean; onClose: () => void; onEdit: () => void; onFocus: (id: string) => void }) {
